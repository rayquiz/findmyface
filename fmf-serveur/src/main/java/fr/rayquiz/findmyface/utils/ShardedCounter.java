 ...
 * 
 * Capable of incrementing the counter and increasing the number of shards. When incrementing, a random shard is
 * selected to prevent a single shard from being written too frequently. If increments are being made too quickly,
 * increase the number of shards to divide the load. Performs datastore operations using the low level datastore API. <br />
 * 
 * @see https://sites.google.com/site/io/building-scalable-web-applications-with-google-app-engine
 */
public class ShardedCounter {

    /**
     * Convenience class which contains constants related to a named sharded counter. The counter name provided in the
     * constructor is used as the entity key.
     */
    ...
    /**
     * Convenience class which contains constants related to the counter shards. The shard number (as a String) is used
     * as the entity key.
     */
    private static final class CounterShard {
        /**
         * Entity kind prefix, which is concatenated with the counter name to form the final entity kind, which
         * represents counter shards.
         */
     ...
     */
    public synchronized final void addShards(final int count) {
    ...
    /**
     * Increment datastore property value inside a transaction. If the entity with the provided key does not exist,
     * instead create an entity with the supplied initial property value.
     * 
     * @param key
