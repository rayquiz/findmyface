...
public class Phonetic {
     ...
     * 
     * On formate d'abord le mot. Remplacement des accents et caractères spéciaux, puis passage en majuscules. Certaines
     * occurences linguistiques sont traitées, puis on supprime toutes les répétitions de lettres. Quelques cas
     * particuliers sont traités. Ce sont des mots très courts qui supportent mal la simplification.
     * 
     * Viennent ensuite les pré-traitements. Ce sont principalement des terminaisons muettes et des combinaisons
     * complexes de voyelles. La grosse partie de la conversion peut commencer. Simplification des sons YEU, K, WEU,
     * GUEU, SION, UN, AN, O ainsi que les muettes.
     * 
     ...
     * 
     * Certains mots courts auront beaucoup souffert de la simplification. C'est pourquoi en début et fin d'algorithme
     * (avant les terminaisons), on aura gardé une copie du code obtenu. On tente alors une "récupération" du code. Si
     * le code n'est pas trop court, on le renvoie. Si le code n'est composé que de consonnes, on suppose alors que
     * c'est un sigle, qu'on renvoie aussi.
     * 
     * 
     * Il faut bien comprendre que cet algorithme a été développé de façon empirique, avec un corpus de test et une
     * grosse part d'intuition, car il est aussi prévu pour fonctionner avec des mots mal orthographiés et/ou inconnus.
     * D'où son comportement de conversion en phonétique. Et quoi qu'il en soit, ce programme n'est qu'une petite partie
     * d'un moteur de recherche. Il y a beaucoup d'astuces à ajouter pour réaliser un moteur de recherche performant.
     * Bon courage!
     */

    public static String genererPhonetic(final String str) {

        String[][] conversion = { { "é", "É" }, { "è", "È" }, { "ë", "Ë" }, { "ê", "Ê" }, { "á", "Á" }, { "â", "Â" },
                { "à", "À" }, { "Ä", "A" }, { "Â", "A" }, { "å", "Å" }, { "ã", "Ã" }, { "æ", "Æ" }, { "ï", "Ï" },
                { "î", "Î" }, { "ì", "Ì" }, { "í", "Í" }, { "ô", "Ô" }, { "ö", "Ö" }, { "ò", "Ò" }, { "ó", "Ó" },
                { "õ", "Õ" }, { "ø", "Ø" }, { "œ", "Œ" }, { "ú", "Ú" }, { "ù", "Ù" }, { "û", "Û" }, { "ü", "Ü" },
                { "ç", "Ç" }, { "ñ", "Ñ" }, { "ß", "S" } };
        String soundex = str;
        ...
        conversion = new String[][] { { "É", "E" }, { "È", "E" }, { "Ë", "E" }, { "Ê", "E" }, { "Á", "A" },
                { "À", "A" }, { "Ä", "A" }, { "Â", "A" }, { "Å", "A" }, { "Ã", "A" }, { "Æ", "E" }, { "Ï", "I" },
                { "Î", "I" }, { "Ì", "I" }, { "Í", "I" }, { "Ô", "O" }, { "Ö", "O" }, { "Ò", "O" }, { "Ó", "O" },
                { "Õ", "O" }, { "Ø", "O" }, { "Œ", "OEU" }, { "Ú", "U" }, { "Ù", "U" }, { "Û", "U" }, { "Ü", "U" },
                { "Ñ", "N" }, { "Ç", "S" }, { "¿", "E" } };

        for (int i = 0; i < conversion.length; i++) {
            soundex = soundex.replaceAll(conversion[i][0], conversion[i][1]);
        }
        // $sIn = utf8_decode($sIn); // Selon votre implémentation, vous aurez besoin de décoder ce qui arrive pour les
        // caractères spéciaux
        ...
        conversion = new String[][] { { "DILAI", "DIAI" }, { "DILON", "DION" }, { "DILER", "DIER" },
                { "DILEM", "DIEM" }, { "RILON", "RION" }, { "TAILE", "TAIE" }, { "GAILET", "GAIET" },
                { "AILAI", "AIAI" }, { "AILAR", "AIAR" }, { "OUILA", "OUIA" }, { "EILAI", "AIAI" },
                { "EILAR", "AIAR" }, { "EILER", "AIER" }, { "EILEM", "AIEM" }, { "REILET", "RAIET" },
                { "EILET", "EIET" }, { "AILOL", "AIOL" } };
        ...
        // MP MB -> NP NB
        conversion = new String[][] { { "OMB", "ONB" }, { "AMB", "ANB" }, { "OMP", "ONP" }, { "AMP", "ANP" },
                { "IMB", "INB" }, { "EMP", "ANP" }, { "GEMB", "JANB" }, { "EMB", "ANB" }, { "UMBL", "INBL" },
                { "CIEN", "SIAN" } };
        for (int i = 0; i < conversion.length; i++) {
            soundex = soundex.replaceAll(conversion[i][0], conversion[i][1]);
        }
        ...
        soundex = soundex.replaceAll("(YN|RI)CH(OG+|OL+|OC+|OP+|OM+|ARIS|M+|IRO|ONDR)", "$1K$2"); // Ou devant une
                                                                                                  // consonne
        ...
        conversion = new String[][] { { "EUCHA", "EKA" }, { "YCHIA", "IKIA" }, { "YCHA", "IKA" }, { "YCHO", "IKO" },
                { "YCHED", "IKED" }, { "ACHEO", "AKEO" }, { "RCHEO", "RKEO" }, { "RCHES", "RKES" }, { "ECHN", "EKN" },
                { "OCHTO", "OKTO" }, { "CHORA", "KORA" }, { "CHONDR", "KONDR" }, { "CHORE", "KORE" },
                { "MACHM", "MAKM" }, { "BRONCHO", "BRONKO" }, { "LICHOS", "LIKOS" }, { "LICHOC", "LIKOC" } };
        for (int i = 0; i < conversion.length; i++) {
            soundex = soundex.replaceAll(conversion[i][0], conversion[i][1]);
        }

        // Weuh (perfectible)
        conversion = new String[][] { { "WA", "OI" }, { "WO", "O" }, { "WI", "OUI" }, { "WHI", "OUI" },
                { "WHY", "OUI" }, { "WHA", "OUA" }, { "WHO", "OU" } };
        for (int i = 0; i < conversion.length; i++) {
            soundex = soundex.replaceAll(conversion[i][0], conversion[i][1]);
        }

        conversion = new String[][] { { "GNES", "NIES" }, { "GNET", "NIET" }, { "GNER", "NIER" }, { "GNE", "NE" },
                { "GI", "JI" }, { "GNI", "NI" }, { "GNA", "NIA" }, { "GNOU", "NIOU" }, { "GNUR", "NIUR" },
                { "GY", "JI" }, { "OUGAIN", "OUGIN" }, { "AGEOL", "AJOL" }, { "AGEOT", "AJOT" }, { "GEOLO", "JEOLO" },
                { "GEOM", "JEOM" }, { "GEOP", "JEOP" }, { "GEOG", "JEOG" }, { "GEOS", "JEOS" }, { "GEORG", "JORJ" },
                { "GEOR", "JEOR" }, { "NGEOT", "NJOT" }, { "UGEOT", "UJOT" }, { "GEOT", "JEOT" }, { "GEOD", "JEOD" },
                { "GEOC", "JEOC" }, { "GEO", "JO" }, { "GEA", "JA" }, { "GE", "JE" }, { "QU", "K" }, { "Q", "K" },
                { "CY", "SI" }, { "CI", "SI" }, { "CN", "KN" }, { "ICM", "IKM" }, { "CEAT", "SAT" }, { "CE", "SE" },
                { "CR", "KR" }, { "CO", "KO" }, { "CUEI", "KEI" }, { "CU", "KU" }, { "VENCA", "VANSA" },
                { "CA", "KA" }, { "CS", "KS" }, { "CLEN", "KLAN" }, { "CL", "KL" }, { "CZ", "KZ" }, { "CTIQ", "KTIK" },
                { "CTIF", "KTIF" }, { "CTIC", "KTIS" }, { "CTIS", "KTIS" }, { "CTIL", "KTIL" }, { "CTIO", "KSIO" },
                { "CTI", "KTI" }, { "CTU", "KTU" }, { "CTE", "KTE" }, { "CTO", "KTO" }, { "CTR", "KTR" },
                { "CT", "KT" }, { "PH", "F" }, { "TH", "T" }, { "OW", "OU" }, { "LH", "L" }, { "RDL", "RL" },
        ...
        soundex = soundex.replaceAll("GNO([MLTNRKG])", "NIO$1"); // bis -> gnognotte! Si quelqu'un sait le faire en une
                                                                 // seule regexp...

        // TI -> SI v2.0

        conversion = new String[][] { { "BUTIE", "BUSIE" }, { "BUTIA", "BUSIA" }, { "BATIA", "BASIA" },
                { "ANTIEL", "ANSIEL" }, { "RETION", "RESION" }, { "ENTIEL", "ENSIEL" }, { "ENTIAL", "ENSIAL" },
                { "ENTIO", "ENSIO" }, { "ENTIAI", "ENSIAI" }, { "UJETION", "UJESION" }, { "ATIEM", "ASIAM" },
                { "PETIEN", "PESIEN" }, { "CETIE", "CESIE" }, { "OFETIE", "OFESIE" }, { "IPESI", "BUTIA" },
                { "LBUTION", "LBUSION" }, { "BLUTION", "BLUSION" }, { "LETION", "LESION" }, { "LATION", "LASION" },
                { "SATIET", "SASIET" } };
        for (int i = 0; i < conversion.length; i++) {
            soundex = soundex.replaceAll(conversion[i][0], conversion[i][1]);
        }
        ...
        // NASALES
        conversion = new String[][] { { "OMT", "ONT" }, { "IMB", "INB" }, { "IMP", "INP" }, { "UMD", "OND" },
                { "TIENT", "TIANT" }, { "RIENT", "RIANT" }, { "DIENT", "DIANT" }, { "IEN", "IN" }, { "YMU", "IMU" },
                { "YMO", "IMO" }, { "YMA", "IMA" }, { "YME", "IME" }, { "YMI", "IMI" }, { "YMN", "IMN" },
                { "YM", "IN" }, { "AHO", "AO" }, { "FAIM", "FIN" }, { "DAIM", "DIN" }, { "SAIM", "SIN" },
                { "EIN", "AIN" }, { "AINS", "INS" } };
        for (int i = 0; i < conversion.length; i++) {
            soundex = soundex.replaceAll(conversion[i][0], conversion[i][1]);
        }
        ...
        soundex = soundex.replaceAll("([VTLJMRPDSBFKNG])EN([BRCTDKZSVN])", "$1AN$2"); // deux fois pour les motifs
                                                                                      // recouvrants malentendu,
                                                                                      // pendentif, ...
        soundex = soundex.replaceAll("([VTLJMRPDSBFKNG])EN([BRCTDKZSVN])", "$1AN$2"); // si quelqu'un sait faire avec
                                                                                      // une seule regexp!
        ...
        conversion = new String[][] { { "EAU", "O" }, { "EU", "E" }, { "Y", "I" }, { "EOI", "OI" }, { "JEA", "JA" },
                { "OIEM", "OIM" }, { "OUANJ", "OUENJ" }, { "OUA", "OI" }, { "OUENJ", "OUANJ" } };
        for (int i = 0; i < conversion.length; i++) {
            soundex = soundex.replaceAll(conversion[i][0], conversion[i][1]);
        }
        ...
        soundex = soundex.replaceAll("^[H]", ""); // H pseudo muet en début de mot, je sais, ce n'est pas une
                                                  // terminaison
        ...
        soundex = soundex.replaceAll("([^GTR])([AO])NG$", "$1$2N");// terminaisons G muet ANG ONG sauf GANG GONG TANG
                                                                   // TONG
        ...
        soundex = soundex.replaceAll("EN$", "AN"); // terminaisons EN -> AN (difficile à faire avant sans avoir des
                                                   // soucis) Et encore, c'est pas top!
        ...
        if (soundex.length() < 2) {
            ...
            if (soundexBack.matches(("[RFMLVSPJDF][AEIOU]"))) {
                if (soundexBack.length() == 3) return (soundexBack.substring(0, 2));// mots de trois lettres supposés
                                                                                    // simples
                if (soundexBack.length() == 4) return (soundexBack.substring(0, 3));// mots de quatre lettres supposés
                                                                                    // simples
            }
